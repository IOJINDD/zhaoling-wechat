    ...
    <mu-popup position="top"
      ...
      <mu-list>
        <mu-list-item :title="item.paraName"
                      :titleClass="{titleClass: showIndex4 === index}"
                      :toggleIconClass="{titleClass: showIndex4 === index}"
                      v-for="(item, index) of loseRoPick"
                      key="item"
                      v-if="loseRoPick.length > 0"
                      @click="chooseLoPi(item, index)">
          <mu-icon slot="right" value="done" v-show="showIndex4 === index"/>
        </mu-list-item>
      </mu-list>
    </mu-popup>
...
<script>
    ...
    export default {
      ...
      methods: {
        ...
        // 选择种类
        chooseLoPi (item, val) {
          this.showIndex4 = val // 标记选中元素
          this.textArray.splice(0, 1, item.paraName) // 替换文字
          this.$emit('chooseLoPi', item.value)
          this.closePop()
        },
        // 选择类型
        chooseType (item, val) {
          this.showIndex0 = val // 标记选中元素
          this.textArray.splice(1, 1, item.paraName) // 替换文字
        ...
        chooseCity (item, val) {
          console.log(item)
          this.showIndex2 = val
          this.textArray.splice(2, 1, item.city) // 替换文字
      ...
      mounted () {
        this.province = areas
        // 避免重复添加
        if (this.province[0].province !== '不限') {
